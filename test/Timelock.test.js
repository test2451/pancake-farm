...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        this.pie = await PieToken.new({ from: alice });
        this.timelock = await Timelock.new(bob, '28800', { from: alice }); //8hours
    });

    it('should not allow non-owner to do operation', async () => {
        await this.pie.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.pie.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.pie.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.pie.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.pie.owner()).valueOf(), carol);
    });

    it('should also work with MasterChef', async () => {
        this.lp1 = await MockOIP20.new('LPToken', 'LP', '10000000000', { from: minter });
        this.lp2 = await MockOIP20.new('LPToken', 'LP', '10000000000', { from: minter });
        this.syrup = await SyrupBar.new(this.cake.address, { from: minter });
        this.chef = await MasterChef.new(this.pie.address, this.syrup.address, dev, '1000', '0', { from: alice });
        await this.pie.transferOwnership(this.chef.address, { from: alice });
